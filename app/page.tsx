...
>
  <div className="relative bg-[#0a0f1e]/80 backdrop-blur-3xl border border-white/5 rounded-[2.5rem] p-5 md:p-10 shadow-2xl">
    ...
    {/* Lista de Classificação - Resolução Definitiva de Sobreposição e Corte */}
<div className="flex flex-col gap-3 w-full">
  {JORNADA_DATA.classificacao.map((item, index) => (
    <div 
      key={index} 
      className={`relative flex items-center justify-between p-4 md:p-6 rounded-[2rem] transition-all w-full ${
        item.destaque 
          ? "bg-gradient-to-r from-red-600 to-red-900 shadow-xl ring-1 ring-white/20" 
          : "bg-white/[0.03] border border-white/5"
      }`}
    >
      {/* Bloco da Esquerda: Pos + Logo + Nome (com Wrap) */}
      <div className="flex items-center gap-3 md:gap-4 flex-1">
        {/* Posição */}
        <div className={`flex items-center justify-center w-9 h-9 rounded-xl text-base font-black italic shrink-0 ${
        ...
        {/* Nome da Equipa com Flex Wrap - Se não couber, desce de linha mas NÃO CORTA */}
        <div className="flex flex-wrap items-center gap-x-2 min-w-0 overflow-visible">
          <span className={`text-[13px] md:text-base font-black uppercase tracking-tight leading-tight break-words ${
            item.destaque ? "text-white" : "text-slate-200"
          }`}>
            {item.nome}
          </span>
          <span className={`text-[7px] md:text-[8px] font-bold uppercase tracking-[0.1em] opacity-50 block w-full ${
            item.destaque ? "text-white" : "text-slate-500"
          }`}>
      ...
      {/* Bloco da Direita: Pontos (Fixo e Protegido) */}
      <div className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl shrink-0 ml-4 self-center ${
